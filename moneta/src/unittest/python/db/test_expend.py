...
class TestExpend:
    ...
    def test_execute_query(self):
        exp.DbHelper()._Expend__execute_query('some query', (1, 2))
        assert mock_db[-1] == ('some query', (1, 2))

    def test___get_from_db(self):
        res = exp.DbHelper._Expend__get_from_db('query', ('some', 'args'))
        assert res == tuple(mock_db)

    def test_edit_name(self):
        args = ('new', 1)
        query = 'UPDATE expend SET name = %s WHERE id = %s;'
        exp.DbHelper().edit_name(1, 'new')
    ...
    def test_edit_planned_cost(self):
        ...
        args = (200, 1,)
        exp.DbHelper().edit_amount(1, 200)
    ...
    def test_edit_image(self):
        ...
        args = (2, 1)
        exp.DbHelper().edit_image_id(1, 2)
    ...
    def test_delete_expend_for_user(self):
        ...
        args = (1, 3)
        exp.DbHelper().delete_expend_for_user(1, 3)

        assert (query, args) == mock_db[-1]

    def test_get_expend_by_id(self):
        res = exp.DbHelper().get_expend_by_id(1)
